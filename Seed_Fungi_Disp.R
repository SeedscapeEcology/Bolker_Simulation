...
### Fungal Dispersal ###
f_colonies <- 10
f_spores <- 100 # it's probably a lot more(?)
f_dispdist <- 2
p_infection <- 0.25
f_movedist <- 5
...
names(fparent_xy)[names(fparent_xy)=="parent_x"] <- "fparent_x"
names(fparent_xy)[names(fparent_xy)=="parent_y"] <- "fparent_y"  # renaming the columns for consistency
...
foffspr_x <- rep(fparent_xy$fparent_x, each = f_spores) + cos(fangle) * fdist
foffspr_y <- rep(fparent_xy$fparent_y, each = f_spores) + sin(fangle) * fdist
